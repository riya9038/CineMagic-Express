...
export const MovieCard = ({ data }) => {
  return (
    <div className="w-56 h-88 relative rounded-md bg-white flex flex-col items-center justify-between cursor-pointer shadow-md shadow-gray-600 p-2.5 hover:scale-105 transition duration-700 ease-in-out">
      <div className="w-full h-52 rounded-t-md bg-center overflow-hidden">
        <img
          src={
            data?.backdrop_path
        ...
        {data?.original_language === "hi" && (
          <HMobiledataIcon className="absolute top-5 left-5 bg-slate-500" />
        )}
      </div>
      <div className="h-28 w-full text-black text-decoration-none flex justify-between p-2 font-sans bg-slate-300 shadow-md shadow-slate-500">
        <div className="overflow-hidden w-full">
          <div className="flex justify-between items-start gap-3">
            <p className="m-0 text-md font-medium w-4/5">
